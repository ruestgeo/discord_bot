...
const reactroles_utils = require('../_utils/reactroles_utils'); //custom utils located in the _utils directory

/* Must include "func" which takes globals as argument (can ignore it if not needed)
    any valid file will be run after the bot is ready
*/
...
module.exports = {
    ...
    func: async function (globals){
        console.log("        Creating 24hour reactrole maintenance interval");
        globals["reactroles"] = {};
        var rr_interval = setInterval(function(){
            garbage_collection(globals);
        }, maintenance_interval); 
        globals["timers"].push(rr_interval);
