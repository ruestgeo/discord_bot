...
module.exports = {
    ...
    manual: "**--remove-role-conditioned3**  ->  `{\"remove-role\": [\"roleName\", ...] <, \"target\": \"roleName\"> <,  \"has-role\": [[\"group1role\", ...], [\"group2role\", ...], ...]> }` \n" +
    ...
    func: async function (globals, msg, content){
        ...
        for (var _member of list){
            ...
            if(args.hasOwnProperty("has-role")){
                var skip = true;
                for (var rolegroup of args["has-role"]){
                    var hasRoleGroup = true;
                    for (var role of rolegroup){ //check if member has at least all of one role group
