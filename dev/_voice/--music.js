...
const {setShutdownTasks} = utils.setShutdownTasks;
...
/**
 * @typedef MusicInfo
 * @property {String} title YT title
 * @property {String} url YT URL
 * @property {String} orgin original request args
 * @property {String} requester the requester
 * @property {Globals}
 */
...
//#region manual
    manual: "Both the bot and the requester must have sufficient permissions.\n***(only works for youtube links)***\n\n" +
            ...
            "**--music**  ->  `playOne __*music_link*__`\n" +
    ...
    func: async function (globals, msg, content){
        try{ 
            ...
            if (content.includes(' ')){
                ...
                args = content.substring(content.indexOf(' ')+1).trim();
            }
            else {
            ...
            if (queues === undefined){
                ...
                queues = {};
                utils.getStorage().acquire(globals, dbName);
                let stringQueues = utils.getStorage().getMultiple(globals, dbName, utils.getStorage().getKeys(globals, dbName));
            ...
            if ( !queues.hasOwnProperty(server.id) ){ //server not in queues
                ...
                let queue = utils.getStorage().get(globals, dbName, server.id);
                if (queue === null) {
                    ...
                    utils.getStorage().set(globals, dbName, server.id, queue.stringify());
                }
                else {
            ...
            if (volumes === undefined){
                ...
                volumes = {};
                utils.getStorage().acquire(dbNameVolumes);
                let stringVolumes = utils.getStorage().getMultiple(globals, dbNameVolumes, utils.getStorage().getKeys(globals,dbNameVolumes));
            ...
            if ( !volumes.hasOwnProperty(server.id) ){ //server not in volumes
                ...
                playing[server.id] = false;
                let volume = utils.getStorage().get(globals, dbNameVolumes, server.id);
                if (volume === null) {
                    utils.botLogs(globals, "----server didn't have volume in localstorage");
                    volumes[server.id] = 1; //default volume of 1 (100%)
                    utils.getStorage().set(globals, dbNameVolumes, server.id, volumes[server.id].toString());
                }
                else {
            ...
            switch (command){
                case "play":{
                    await play(globals, msg, connection /*, args*/).catch(err => { throw (err) });
                    break;
                }
                case "playOne":{
                    await playOne(globals, msg, connection, info).catch(err => { throw (err) });
                    break;
                }
                case "pause":{
                    await pause(globals, msg).catch(err => { throw (err) });
                    break;
                }
                case "stop":{
                    await stop(globals, msg).catch(err => { throw (err) });
                    break;
                }
                case "next":{
                    await next(globals, msg, msg.guild.id).catch(err => { throw (err) });
                    break;
                }
                case "add":{
                    await add(globals, msg, info).catch(err => { throw (err) });
                    break;
                }
                case "insert":{
                    await insert(globals, msg, info, position).catch(err => { throw (err) });
                    break;
                }
                case "remove":{
                    await remove(globals, msg, info).catch(err => { throw (err) });
                    break;
                }
                case "removeAll":{
                    await removeAll(globals, msg, info).catch(err => { throw (err) });
                    break;
                }
                case "clear":{
                    await clear(globals, msg).catch(err => { throw (err) });
                    break;
                }
                case "list":{
                    await list(globals, msg, args).catch(err => { throw (err) });
                    break;
                }
                case "info":{
                    await getInfo(globals, msg, info).catch(err => { throw (err) });
                    break;
                }
                case "volume":{
                    await setVolume(globals, msg, args).catch(err => { throw (err) });
                    break;
                }
                case "controller":{
...
/**
 * play just the first item in the queue
 ...
 * @returns 
 */
async function playOne (globals, msg, connection, info){
    ...
    streamDispatcher.once("finish", async () => { 
        ...
        if (!info) {
            ...
            utils.getStorage().put(globals, dbName, server_id, queues[server_id].stringify());
        }
    })
    .once("error", onError);
...
/**
 * start audio player, or resume playing
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {*} connection TODO
 * @returns 
 */
async function play (globals, msg, connection /*, args*/){
    let server_id = msg.guild.id;
    ...
    if ( !connection.dispatcher ){ 
        ...
        streamDispatcher.once("finish", () => { console.log("!__[music]  finish -> next ["+server_id+"]");  next(globals, null, server_id); })
        .once("error", onError);
        let server_name = connection.channel.guild.name;
        eventsRemovers[server_id] = utils.event_once(connection, 'disconnect', async () => {
            console.log("        !__[music]  disconnect event on connection for ["+server_name+":"+server_id+"]");
 ...
 * @returns 
 */
async function pause (globals, msg){
    let server_id = msg.guild.id;
 ...
 * @returns 
 */
async function stop (globals, msg){
    let server_id = msg.guild.id;
 ...
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {Discord.Snowflake} server_id
 * @returns 
 */
async function next (globals, msg, server_id){
    let connection = connections[server_id];
    if (!msg && !connection){ 
    ...
    if (nextSong === null){
        ...
        utils.getStorage().set(globals, dbName, server_id, queues[server_id].stringify());
        return;
    }
    if (was_playing){
        ...
        .once("finish", () => { console.log("!__[music]  finish -> next ["+server_id+"]");  next(globals, null, server_id); })
        .once("error", onError);
        playing[server_id] = true;
        streamDispatcher.setVolumeLogarithmic(volumes[server_id]);
    }
...
/**
 * add one or multiple items into the queue
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {MusicInfo|MusicInfo[]} info
 * @returns 
 */
async function add (globals, msg, info){
    let server_id = msg.guild.id;
        ...
        try { for (; index < info.length; index++) queues[server_id].enqueue(info[index]);  }
            ...
            if (all.length+header.length > 2000){
                ...
                while (all.length > 2000){
                    let split_index = all.substring(1800, all.length).indexOf("\n")+1800;
                    parts.push(all.substring(0,split_index));
                    all = all.substring(split_index, all.length);
                }
...
/**
 * insert an item in the queue at position
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {MusicInfo} info
 ...
 * @returns 
 */
async function insert (globals, msg, info, position){
    let server_id = msg.guild.id;
...
/**
 * remove an item in the queue by index or url
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {Number|MusicInfo} arg
 * @returns 
 */
async function remove (globals, msg, arg){
    let server_id = msg.guild.id;
    let removed;
...
/**
 * remove all matching entries from the queue
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {MusicInfo} info
 * @returns 
 */
async function removeAll (globals, msg, info){
    let server_id = msg.guild.id;
 ...
 * @returns 
 */
async function clear (globals, msg){
    let server_id = msg.guild.id;
...
/**
 * list the titles of audio in the queue
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {Number} amount
 * @returns 
 */
async function list (globals, msg, amount){
...
/**
 * return info from the link provided or the current song if no link is provided
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {MusicInfo} info
 * @returns 
 */
async function getInfo (globals, msg, info){
    if (info) {
...
/**
 * set bot volume for the server
 * @param {Globals} globals 
 * @param {Discord.Message} msg 
 * @param {Number} arg
 * @returns 
 */
async function setVolume (globals, msg, arg){
    let server_id = msg.guild.id;
    if (!arg){ //obtain and reply with the current volume
        console.log("__[music]  get volume for bot in server");
        let vol = parseFloat(utils.getStorage().get(globals, dbNameVolumes, server_id));
    ...
    utils.getStorage().set(globals, dbNameVolumes, server_id, volumes[server_id].toString());
    if (connections[server_id]){
        if (connections[server_id].dispatcher){
...
async function createController (globals, msg){
    ...
    await utils.react_controller(globals, {
        requester: "music controller ["+serverID+"]",
        targetMessage: target_msg,
        isAuthorized: async (globals, serverID, userID) => {
            //isAuthorized
        ...
        callbacks: { //react_callbacks
            "▶️" : {
                'callback': async (globals, server_ID, user_ID, token) => {
                    //fetch connection
                    let connection = voiceUtils.fetchLatestConnection(globals.client, server_ID);
                    if (!connection){
                        controllerInfo[server_ID].content = "Currently not connected to a voice channel in this server";
                        return;
                    }
                    await play(globals, fake_message, connection).catch(err => { controllerInfo[server_ID].content = err });
            ...
            "⏸️" : {
                'callback': async (globals, server_ID, user_ID, token) => {
                    await pause(globals, fake_message).catch(err => { controllerInfo[server_ID].content = err });
                    flushControllerContent(controllerInfo, server_ID);
                },
                'awaitLock': false,
                'title': "pause"
            ...
            "⏹️" : {
                'callback': async (globals, server_ID, user_ID, token) => {
                    await stop(globals, fake_message).catch(err => { controllerInfo[server_ID].content = err });
                    flushControllerContent(controllerInfo, server_ID);
                },
                'awaitLock': false,
                'title': "stop"
            ...
            "↩️" : {
                'awaitLock' : true,
                'callback': async (globals, server_ID, user_ID, token) => {
                    await stop(globals, fake_message).catch(err => { 
                    ...
                    await play(globals, fake_message, connection).catch(err => { 
                        controllerInfo[server_ID].content = err;
                        flushControllerContent(controllerInfo, server_ID);
                        return;
                    });
            ...
            "⏭️" : {
                'awaitLock' : true,
                'callback': async (globals, server_ID, user_ID, token) => {
                    await next(globals, fake_message, server_ID).catch(err => { controllerInfo[server_ID].content = err });
                    flushControllerContent(controllerInfo, server_ID);
                },
                'awaitLock': false,
                'title': "next"
            ...
            "🔼" : {
                'callback': async (globals, server_ID, user_ID, token) => {
                    ...
                    await setVolume(globals, fake_message, (currentVolume + 0.1)*100).catch(err => { controllerInfo[server_ID].content = err });
                    flushControllerContent(controllerInfo, server_ID);
                },
                'awaitLock': false,
                'title': "volume+"
            ...
            "🎵" : {
                'callback': async (globals, server_ID, user_ID, token) => {
                    await list(globals, fake_message, 10).catch(err => { controllerInfo[server_ID].content = err });
                    flushControllerContent(controllerInfo, server_ID);
                },
                'awaitLock': false,
                'title': "list songs"
            ...
            "❌" : {
                ...
                'awaitLock': false,
                'title': "destroy controller"
            },
        }
    });
    if (setShutdown){
        setShutdownTasks("--music", async () => {
