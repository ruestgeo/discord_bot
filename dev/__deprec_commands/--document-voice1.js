...
module.exports = {
    ...
    manual: "**--document-voice**  ->  `channel_id`   *or*   \\``channel_name`\\` \n" +
            ".     *Dumps the member information (names) that are in a specified voice channel (via ID, or by name if encapsulated with backtick/grave-accent-mark) into a specified google sheet*",
    ...
    func: async function (globals, msg, content){
        ...
        await gs_utils.dumpToSheet(msg, globals, channel_title, list, 0, col.length, 0, 2)
        .catch (err => { throw (err); });
    }
